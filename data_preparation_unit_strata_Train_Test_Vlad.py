...
def df_train_creator(df_all, units_index_train):
    ...
    for idx in units_index_train:
        ...
        #total_trainable_sample_per_unit = ratio_per_unit*total_train_all_units
       ...
       #Taking stratified random samples for each unit
        ...
        for rul in count_values:
            ...
            df_test_temp_1 = df_vlad_test_temp_1.drop(df_vlad_temp_2.index)
  
            # here we are taking random samples from the returned Dataframe to have the exact amount required for testing
            sample_per_RUL_test=int(round(rul/total_data_point_per_unit*3550, 0 ))
            df_test_temp_2 = df_test_temp_1[df_test_temp_1['RUL']==np.float64(B)]
            #Sampling randomly for each required rows per RUL for testing
            df_test_temp_2 = df_test_temp_2.sample(n=sample_per_RUL_test,axis =0)
            ...
            df_test=pd.concat(test_strata)
            #Sorting the testing
            # dataframe in ascending order with respect to unit number, RUL number and alt number
            df_test = df_test.sort_values(by = ["unit","RUL","alt"], ascending=[True, False, True])
        ...
        #i need the df_test to run the same code for ration sampling to get real df_test


    return df_train,df_vlad,df_test
