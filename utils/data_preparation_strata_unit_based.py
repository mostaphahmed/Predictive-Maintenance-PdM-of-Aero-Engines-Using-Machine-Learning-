...
def df_all_creator(data_filepath, sampling):
    ...
    # Most Correlated 10  
    # df_W = pd.DataFrame(data=W[:,0:2], columns=['alt','Mach'])
    # df_W = pd.DataFrame(data=W, columns=W_var).drop(columns = ['alt','Mach','TRA','T2'])
    # df_Xs = pd.DataFrame(data=X_s, columns=X_s_var).drop(columns = ['Nc','T24','T48','T50','P24','P50','T30','P15', 'P2', 'P21','Ps30', 'P40','Nf'])
    # df_Xv = pd.DataFrame(data=X_v,columns= X_v_var).drop(columns=['T40','P30', 'P45', 'W21', 'W22', 'W25', 'W31', 'W32', 'W48', 'W50'])
    # df_T = pd.DataFrame(data=T, columns=T_var).drop(columns = ['fan_eff_mod', 'fan_flow_mod', 'LPC_eff_mod', 'LPC_flow_mod', 'HPC_eff_mod', 'HPC_flow_mod','HPT_flow_mod'])
    # df_Y = pd.DataFrame(data=Y, columns=['RUL'])
    # #df_A = pd.DataFrame(data=A, columns=A_var) 
    # df_A = pd.DataFrame(data=A, columns=A_var).drop(columns=['cycle'])
    
    df_W = pd.DataFrame(data=W, columns=W_var)    # Scenario Descriptors 
    df_Xs = pd.DataFrame(data=X_s, columns=X_s_var) # Mesurnment Sensors 
    df_Xv = pd.DataFrame(data=X_v[:,0:2], columns=['T40', 'P30']) # Virtual Sensors
    #df_T = pd.DataFrame(data=T, columns=T_var) # Model Health Parameters
    df_Y = pd.DataFrame(data=Y, columns=['RUL'])  # True RUL Labels
    df_A = pd.DataFrame(data=A, columns=A_var).drop(columns=['cycle', 'Fc', 'hs']) # Auxilary Data
      
    # Merge all the dataframes
    df_all = pd.concat([df_W, df_Xs, df_Xv, df_A, df_Y], axis=1)
...
# def df_train_creator(df_all, units_index_train): # PCA Included
    train_strata = []
    vlad_strata = []
    global pca, scaler
    ...
    for idx in units_index_train:
        ...
        #Ratio carried by unit
        # ratio_per_unit= total_data_point_per_unit/total_samp

        #Total sample for training per unit
        # total_trainable_sample_per_unit = ratio_per_unit*percentage_total_sam
       ...
       #Taking stratified random samples for each unit
        ...
        for rul in count_values:
            ...
            sample_per_RUL_train=int(round(rul/total_data_point_per_unit*1506, 0 ))# # For (8,1) Train_per_unit ==  13506 to get 108046 (total)
            ...
            sample_per_RUL_vlad=int(round(rul/total_data_point_per_unit*1131, 0 )) # For (8,1) Vald_per_unit ==  10231 to get 81450 (total)
            ...
            #Train_RUL = df_train.iloc[:,36].reset_index(drop = True)
...
# def df_test_creator(df_all, units_index_test): # PCA Included
    test_strata= []
    ...
    for idx in units_index_test:
       ...
       #Taking stratified random samples for each unit
        ...
        for rul in count_values:
            #Getting required samples for each RUL
            sample_per_RUL_test=int(round(rul/total_data_point_per_unit*443, 0 ))  # For (8,1) Test_per_unit ==  31950 to get 31950 (total) 20390
            ...
            # Test_RUL = df_test['RUL'].to_numpy()
            # Test_Featuers = df_test.drop(columns = ['RUL']).to_numpy()
            Test_Unit = df_test['unit'].reset_index(drop = True)
            Test_Featuers = df_test.iloc[:,0:36].reset_index(drop = True)
            #Test_RUL = df_test.iloc[:,36].reset_index(drop = True)
            ...
            #print ("df_test_scaled", test_scaled)
            # Applying PCA 
...
def df_train_creator(df_all, units_index_train):
    ...
    global df_train, df_train_temp, df_train_concate, count_values, df_train_temp_unit_2, df_train_temp_2,df_train_1, sample_per_RUL_train, df_train_temp_1
    for idx in units_index_train:
        # selecting the training units
        df_train_temp = df_all[df_all['unit'] == np.float64(idx)]
       ...
       #Taking stratified random samples for each unit
        ...
        for rul in count_values:
            ...
            sample_per_RUL_train = int(round(rul/total_data_point_per_unit*12050, 0 ))# For (6,3) 18050 for(7,2) 15497 #(6,3) 18050  All Units 12050 (7,2) 15479
            df_train_temp_1 = df_train_temp[df_train_temp['RUL']==np.float64(B)]
            #print(df_train_temp_1)
            #Sampling randomly for each required rows per RUL for training
            df_train_temp_2 = df_train_temp_1.sample(n=sample_per_RUL_train,axis =0)
            #Getting the resultant of the dataframe for testing
            df_vlad_temp_1 = df_train_temp_1.drop(df_train_temp_2.index)

            # here we are taking random samples from the returned Dataframe to have the exact amount required for testing
            sample_per_RUL_vlad=int(round(rul/total_data_point_per_unit*9050, 0 )) # For (6,3) 13550 (7,2) FOr (6,3) 13550 All Units 9050 # (7,2) 11626
            df_vlad_temp_2 = df_vlad_temp_1[df_vlad_temp_1['RUL']==np.float64(B)]
            #Sampling randomly for each required rows per RUL for testing
            df_vlad_temp_2 = df_vlad_temp_2.sample(n=sample_per_RUL_vlad,axis =0)
